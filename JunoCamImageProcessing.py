...
def main():
    filename = './images/JNCR_2021052_32C00032_V01.IMG' # change the filename to the name of the .IMG that is being processed
    ...
    framelet = framelets[21] # change this index to whichever framelet to process
...
def rectify_limb(framelet, limb_endpoints, x_step, y_step, width, height):
    ...
    for p in range(len(limb_endpoints)):
        if p > 0:
            ...
            #if indices of boundary rectangle are out of bounds, disregard segment
...
def label_haze(framelet, boundary_points):
    ...
    for b in range(len(boundary_points)):
        ...
        for i in range(23, 38):
            ...
            for j in range(20):
                index_x = int(starting_point_x + j / 20.0 * diff_row_x)
                index_y = int(starting_point_y + j / 20.0 * diff_row_y)
